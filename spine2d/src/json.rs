            ...
            while !remaining.is_empty() && passes_left > 0 {
                ...
                for pending in remaining {
                    ...
                    let Some(parent_attachment) =
                        parent_skin.attachment(pending.source_slot_index, &pending.source)
                    else {
                        return Err(Error::JsonInvalidMeshData {
                            skin: pending.skin.clone(),
                            slot: slots
                    ...
                    if pending.inherit_deform && pending.slot_index != pending.source_slot_index {
                        let Some(parent_skin) = skins.get_mut(&parent_skin_name) else {
                            continue;
                        };
                        ...
                        let Some(parent_attachment) =
                            parent_slot_map.get_mut(pending.source.as_str())
                        else {
                            continue;
                        };
