...
//
// Notes:
// - This specifically exercises additive `applyMixingFrom` with an attachment timeline that
//   the incoming animation does not retain. Official Spine 4.3 clears `crosshair` here.
// - The `shoot` animation uses the upstream JSON key `rgba` for slot color timelines.
#[test]
fn example_spineboy_aim_to_shoot_additive_mixing_matches_official_attachments_and_rgba_colors() {
    let mut h = SpineboyHarness::new(|state_data| {
